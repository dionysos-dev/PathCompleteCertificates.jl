...
The sampling half is already here: `simulate` runs a system along one switching sequence and
returns a `Trajectory`. It is filed under `src/systems/` rather than with the problems,
because nothing about it certifies. `refute` is that plus a property to check and a search over
sequences.
